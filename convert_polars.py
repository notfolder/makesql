...
def convert_parquet_to_wide_csv_zip_polars(parq_root_dir, output_zip_path, output_csv_name='all_serials.csv'):
    t_start = time.time()
    ...
    t1 = time.time()
    print("[TIMER] polarsでparquet一括読み込み開始")
    df = pl.read_parquet(parquet_files)
    print(f"[TIMER] polarsでparquet一括読み込み完了, 経過: {time.time()-t1:.2f}秒")
    ...
    t3 = time.time()
    print("[TIMER] CSV出力開始")
    wide.write_csv(output_csv_name)
...
if __name__ == "__main__":
    ...
    output_zip = 'wide_csvs_polars.zip'
    print(f"変換開始: {parq_root} → {output_zip}")
    t0 = time.time()
    convert_parquet_to_wide_csv_zip_polars(parq_root, output_zip)
    t1 = time.time()
